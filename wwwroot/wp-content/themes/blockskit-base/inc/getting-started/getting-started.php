...
class Bk_Notice_Handler {
    ...
    public function bk_theme_info_welcome_admin_notice() {
        if ( is_admin() && !get_user_meta( get_current_user_id(), 'gs_notice_dismissed' ) ){
            ...
            echo ( '<p><strong>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s.', 'blockskit-base' ), $this->theme_name ) . '</strong></p><p class="plugin-notice">'.esc_html__( 'By clicking "Get Started," you will install and activate Blockskit Import & Advanced Import plugins to prepare your demo import.', 'blockskit-base' ).'</p><p><a href="#" class="bk-install-plugins button button-primary">' . sprintf( esc_html__( 'Get started with %s','blockskit-base' ), $this->theme_name ) . '</a></p>' );
            }
	...
	public function bk_getting_started() {
        ...
        if( $request == 2 && strpos( $slug, 'blockskit-base' ) === false ){
            wp_send_json_error( );
        }
